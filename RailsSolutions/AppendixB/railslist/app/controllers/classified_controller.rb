class ClassifiedController < ApplicationController
  ...
  def update 
    ...
    if @classified.update_attributes(params[:classified]) 
      expire_action(:controller => "classifieds", :action => "show"), :id => @classified 
  ...
  def email 
    ...
    if request.post? 
      ClassifiedMailer.deliver_classified_with_attachment(➥ 
      params[:user][:email], @classified, url) 
    end 
  end 
